...
void AR_WPNav_OA::update(float dt)
{
    ...
    // Patrol stopping lines own immutable geometry and bypass OA replanning.
    if (_stopping_line_oa_bypass) {
        AR_WPNav::update(dt);
        return;
    }
...
bool AR_WPNav_OA::set_desired_location(const Location& destination, Location next_destination)
{
    ...
    _oa_active = false;
    _stopping_line_oa_bypass = false;
    return true;
}

bool AR_WPNav_OA::set_desired_location_stopping_from_origin(
    const Location &origin,
    const Location &destination,
    float speed_max_mps)
{
    if (!AR_WPNav::set_desired_location_stopping_from_origin(
            origin,
            destination,
            speed_max_mps)) {
        return false;
    ...
    _oa_active = false;
    _stopping_line_oa_bypass = true;
    return true;
}

void AR_WPNav_OA::cancel_stopping_line()
{
    AR_WPNav::cancel_stopping_line();
    _stopping_line_oa_bypass = false;
    _oa_active = false;
}
...
{
    // OA should always be inactive before reaching the final destination.
    return !_oa_active && AR_WPNav::reached_destination();
}
...
{
    return _oa_active ? _oa_origin : _origin;
}
...
{
    return _oa_active ? _oa_destination : AR_WPNav::get_oa_destination();
}
