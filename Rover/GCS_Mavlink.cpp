...
{
    const float motor1 = 10000 * (SRV_Channels::get_output_scaled(SRV_Channel::k_throttleLeft) * 0.001f);
    const float motor3 = 10000 * (SRV_Channels::get_output_scaled(SRV_Channel::k_throttleRight) * 0.001f);
...
{
    return rover.gcs().sysid_gcs();
}

bool GCS_MAVLINK_Rover::sysid_enforce() const
{
    return rover.gcs().option_is_enabled(GCS::Option::GCS_SYSID_ENFORCE);
}

uint32_t GCS_MAVLINK_Rover::telem_delay() const
{
    return rover.gcs().telem_delay();
}
