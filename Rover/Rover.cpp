...
const AP_Scheduler::Task Rover::scheduler_tasks[] = {
    ...
    SCHED_TASK(update_dist,           10,    200,   6),
...
void Rover::one_second_loop(void)
{
    ...
    // attempt to update home position if not armed:
...
void Rover::update_current_mode(void)
{
    ...
    control_mode->update();
    update_exact_pivot_diagnostics();
}

void Rover::update_exact_pivot_diagnostics()
{
    const AR_WPNav::ExactPivotDiagSnapshot snapshot =
        g2.wp_nav.get_exact_pivot_diag_snapshot();
    const uint16_t transitions = snapshot.transition_flags;
    const uint16_t active_mask = AR_WPNav::DiagStateExactActive |
                                 AR_WPNav::DiagStateCompletion |
                                 AR_WPNav::DiagStateRejoinActive |
                                 AR_WPNav::DiagStateHandoffActive;
    const bool state_relevant = (snapshot.state_flags & active_mask) != 0U;
    const bool relevant = (transitions != 0U) || state_relevant;
    const bool exit_edge = !relevant &&
                           exact_pivot_diag_monitor.was_state_relevant;
    if (!relevant && !exit_edge) {
        return;
    }

    const uint64_t now_us = AP_HAL::micros64();
    const bool phase_changed = snapshot.phase != exact_pivot_diag_monitor.last_phase;
    const bool fault_changed = snapshot.fault != exact_pivot_diag_monitor.last_fault;
    const bool generation_changed =
        snapshot.handoff_generation != exact_pivot_diag_monitor.last_generation;
    const bool force = (transitions != 0U) || exit_edge || phase_changed ||
                       fault_changed || generation_changed;
    constexpr uint32_t log_interval_us = 40000U; // 25Hz
    if (!force &&
        ((now_us - exact_pivot_diag_monitor.last_log_us) < log_interval_us)) {
        return;
    }

    // Mission Planner receives concise, one-shot transition messages.  The
    // continuous numeric stream remains in XPNV/XCAP/XHOF DataFlash records.
    if ((transitions & AR_WPNav::DiagNewExact) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP New id=%lu h=%.1f",
                        (unsigned long)snapshot.leg_id,
                        (double)snapshot.target_heading_deg);
    }
    if ((transitions & AR_WPNav::DiagEarlyCaptureA) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP CapA d=%.2f p=%.2f v=%.2f",
                        (double)snapshot.endpoint_distance_m,
                        (double)snapshot.planned_distance_m,
                        (double)snapshot.planned_speed_mps);
    }
    if ((transitions & AR_WPNav::DiagCapturePathB) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP CapB L=%.2f R=%.2f dir=%d",
                        (double)snapshot.capture.length_m,
                        (double)snapshot.capture.radius_m,
                        (int)snapshot.capture.direction);
    }
    if ((transitions & AR_WPNav::DiagEnterSpin) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP Spin src=%c dir=%d e=%.1f",
                        snapshot.spin_source == 0U ? '?' : (int)snapshot.spin_source,
                        (int)snapshot.turn_direction,
                        (double)snapshot.yaw_error_deg);
    }
    if ((transitions & AR_WPNav::DiagPromotePath) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP Promote g=%lu rj=%.2f bl=%.2f",
                        (unsigned long)snapshot.handoff_generation,
                        (double)snapshot.handoff.rejoin_m,
                        (double)snapshot.handoff.blend_m);
    }
    if ((transitions & AR_WPNav::DiagAckOrdinary) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP AckOrd g=%lu",
                        (unsigned long)snapshot.acknowledged_generation);
    }
    if ((transitions & AR_WPNav::DiagAckExact) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP AckExact g=%lu h=%.1f",
                        (unsigned long)snapshot.acknowledged_generation,
                        (double)snapshot.target_heading_deg);
    }
    if ((transitions & AR_WPNav::DiagHandoffDone) != 0U) {
        gcs().send_text(MAV_SEVERITY_INFO,
                        "WPNav XP Handoff done g=%lu",
                        (unsigned long)snapshot.handoff_generation);
    }
    if ((transitions & AR_WPNav::DiagOACancel) != 0U) {
        gcs().send_text(MAV_SEVERITY_WARNING,
                        "WPNav XP OA-cancel ph=%u why=%u",
                        (unsigned)snapshot.oa_cancel_from_phase,
                        (unsigned)snapshot.oa_cancel_reason);
    }

#if HAL_LOGGING_ENABLED
    if (should_log(MASK_LOG_NTUN)) {
        Log_Write_Exact_Pivot_Nav(snapshot, now_us, force);
        const bool capture_record = snapshot.capture.valid &&
            (((snapshot.state_flags & AR_WPNav::DiagStateCaptureActive) != 0U) ||
             ((transitions & (AR_WPNav::DiagCapturePathB |
                              AR_WPNav::DiagEnterSpin |
                              AR_WPNav::DiagFault |
                              AR_WPNav::DiagOACancel)) != 0U));
        if (capture_record) {
            Log_Write_Exact_Pivot_Capture(snapshot, now_us, force);
        }
        const bool handoff_record = snapshot.handoff.valid &&
            (((snapshot.state_flags & (AR_WPNav::DiagStateRejoinActive |
                                       AR_WPNav::DiagStateHandoffActive)) != 0U) ||
             ((transitions & (AR_WPNav::DiagPromotePath |
                              AR_WPNav::DiagHandoffDone |
                              AR_WPNav::DiagFault |
                              AR_WPNav::DiagOACancel)) != 0U));
        if (handoff_record) {
            Log_Write_Exact_Pivot_Handoff(snapshot, now_us, force);
        }
    }
#endif

    exact_pivot_diag_monitor.last_log_us = now_us;
    exact_pivot_diag_monitor.last_phase = snapshot.phase;
    exact_pivot_diag_monitor.last_fault = snapshot.fault;
    exact_pivot_diag_monitor.last_generation = snapshot.handoff_generation;
    exact_pivot_diag_monitor.was_state_relevant = state_relevant;
    if (transitions != 0U) {
        g2.wp_nav.ack_exact_pivot_diag(snapshot.transition_sequence);
    }
}
