...
function update()
  ...
  local update_rfnd = false  -- gcs:send_text(0, "Distance:"..tostring(distance).." Instance:"..tostring(instance))
  ...
  if status == 0 then
    ...
    if (SET_PRX:get() == 1) then
      ...
      -- gcs:send_text(0, string.format("TOFSENSE-M %d: Distance: %.2f m, Yaw: %.2f, Pitch: %.2f", instance, distance, yaw, pitch))
    end
